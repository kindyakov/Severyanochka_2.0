...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($fileName) && !empty($name) && !empty($id) && !empty($rout)) {
    ...
    if (file_exists($templateFile)) {
      ...
      if ($rout === 'type') {
        $responseData = updateTypeFile($fileName, $htmlContent);
      }
      if ($rout === 'product') {
        // $responseData = updateProductFile($fileName, $htmlContent, $nameType, $responseData);
      }
    } else {
...
function updateTypeFile($fileName, $content)
{
  ...
  // Обновление папки
  $responseData = updateFolder($fileName, $directory);
  return $responseData;
}

function updateFolder($fileName, $directory)
{
